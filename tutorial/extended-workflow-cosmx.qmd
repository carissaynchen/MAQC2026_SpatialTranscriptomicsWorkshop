...
plotZoomFovsMap(cos, 
    mapPointCol="green",
    colourBy="Mean.DAPI",
    fovs=c(98:100, 113:115, 122:124), scaleBarMap=FALSE)
```
...
Using `r BiocStyle::Biocpkg("SpaceTrooper")`'s quality score, we will filter the CosMx dataset
...
## Which cell is considered a neighbour?

The `findSpatialNeighbors` function, using the `poly2nb` method builds a neighbours list based on polygons with contiguous boundaries, that is sharing one or more boundary points. Boundary points less than `snap` distance apart are considered to indicate contiguity; used both to find candidate and actual neighbours. The default condition to be considered a 'neighbour cell' requires that at least one single shared boundary point meets the contiguity condition.
